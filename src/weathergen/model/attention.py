...
# Maximum number of sequences per flash_attn_varlen_func call.
_VARLEN_MAX_SEQS = 65534


def _flash_attn_varlen_chunked(
    q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, **kwargs
):
    """Automatically chunks the call when the number of sequences exceeds the CUDA limit.
    ...
    if num_seqs <= _VARLEN_MAX_SEQS:
        return flash_attn_varlen_func(
            q, k, v, cu_seqlens_q, cu_seqlens_k, max_seqlen_q, max_seqlen_k, **kwargs
        )

    # --- Chunked path based purely on sequence indices ---
    chunk_boundaries = list(range(0, num_seqs, _VARLEN_MAX_SEQS))
    if chunk_boundaries[-1] != num_seqs:
        chunk_boundaries.append(num_seqs)

    num_chunks = len(chunk_boundaries) - 1
    logger.debug("flash_attn_varlen_func: chunking %d sequences into %d chunks", num_seqs, num_chunks)

    # Query all sequence token boundaries at the chunk edges in a single GPU->CPU transfer
